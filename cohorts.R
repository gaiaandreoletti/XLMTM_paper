...
## differentially expressed genes in all comparisons between samples of both cohorts (i.e., #1 and #2).
...
## Results (files and plots) are saved into the directory "../results/cohort1&cohort2".
...
### directories and libraries ###
  ...
  res.dir<-file.path(main.res.dir,paste0(sub.column1,"&",sub.column2))
...
### sub-sample info file and data matrix ###
  infos<-sample.infos[sample.infos[,sub.column1]=="yes"|sample.infos[,sub.column2]=="yes",]
  ...
  ### Compute counts per million (CPM) and moderated log2(CPM) ### 
    ...
    write.table(cpm.matrix,file=paste0(sub.column1,"&",sub.column2,"_cpm.txt"),sep="\t",quote=F,row.names=F)
    cpm.log.matrix.to.write<-data.frame("GeneSymbols"=rownames(cpm.log.matrix),cpm.log.matrix)
    write.table(cpm.log.matrix,file=paste0(sub.column1,"&",sub.column2,"_log2cpm.txt"),col.names=NA,sep="\t",quote=F)
  ...
  ### Compute fragment per kilobase (FPKM) if pair-end or read per kilobase (RPKM) if single-end ### 
    ...
    write.table(rpkm.matrix,file=paste0(sub.column1,"&",sub.column2,"_rpkm.txt"),sep="\t",quote=F,row.names=F)
  ...
  ### unsupervised analysis: clustering and PCA ###
    ...
    autoplot(pca,x=1,y=2,data=infos,colour='time',scale=0,shape="dose_level",
    ...
    annot.col<-data.frame(Time=infos$time,Tissue=infos$biopsy_tissue,Mutation=infos$mutation_type,Dose=infos$dose_level)
    ...
    ann.colors<-list(Time=Time,Tissue=Tissue,Mutation=Mutation,Dose=Dose)
  ...
  ### Compute DEGs ###
    ...
    for (i in 1:dim(comparisons)[1]){
      ...
      keep<-table(sub.infos$patient)==2
      sub.infos<-sub.infos[sub.infos$patient%in%names(table(sub.infos$patient))[keep],]
      ...
      degs<-data.frame(rownames(DGE.obj),degs$table[rownames(DGE.obj),c(7,2,4:6)])
      colnames(degs)[1]<-"GeneSymbol"
      length(rownames(degs[degs$FC>=2&degs$FDR<=0.05,]))
      length(rownames(degs[degs$FC<=-2&degs$FDR<=0.05,]))
      write.table(degs,file=paste0(sub.column1,"&",sub.column2,"_",time2,"-vs-",time1,".txt",sep=""),sep="\t", row.names = F)
      # Clustering on DEGs ##
      ...
      ggsave(file=paste0(sub.column1,"&",sub.column2,"_DEG_",time2,"-vs-",time1,".pdf"),g,width=12,height=7,useDingbats=FALSE)
      dev.off()
    }
...
### creation of files for GSEA ###
  ...
  # creation of .cls file #
    ...
    filename<-paste(sub.column1,"&",sub.column2,".cls",sep="")
